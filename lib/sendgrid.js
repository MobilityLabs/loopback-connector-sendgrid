...
var {MailService} = require('@sendgrid/mail');
var helper = require('@sendgrid/helper.classess');
...
var SendgridConnector = function SendgridConnector(settings) {
  ...
  if (settings.api_key){
    ...
    this.sendgrid.setApiKey(settings.api_key); //eslint-disable-line
  }
...
Mailer.send = function (options, cb) { // eslint-disable-line
  ...
  if (connector.sendgrid) {
    ...
    if (R.is(Array, options.to)) {
      sendgridMessage.to = R.map(
        function unwrapArrayString(recipient) {
          ...
          return new helper.classes.EmailAddress(recipient.email, recipient.name);
        },
        options.to
      );
    } else {
    ...
    sendgridEmail = new helper.classes.Mail(
      new helper.classes.EmailAddress(sendgridMessage.from, sendgridMessage.fromname),
      options.subject,
      sendgridMessage.to[0] // needs major refactoring to send to each 'to'
    );
    if (options.html) {
      sendgridEmail.addContent(new helper.classes.Content("text/html", options.html));
    }
    ...
    if (options.sendGridConfig) {
      ...
      if (R.is(Object, options.sendGridConfig.dynamic_template_data)) {
        sendgridEmail.dynamic_template_data = options.sendGridConfig.dynamic_template_data; // eslint-disable-line
      }

    }

    connector.sendgrid.send(sendgridEmail, function sgCb(response){ // eslint-disable-line
